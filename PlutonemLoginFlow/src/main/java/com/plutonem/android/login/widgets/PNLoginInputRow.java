 ...
 */
public class PNLoginInputRow extends RelativeLayout {
    ...
    public PNLoginInputRow(Context context) {
        super(context);
        init(context, null);
    }

    public PNLoginInputRow(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public PNLoginInputRow(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.pnLoginInputRow, 0, 0);

            try {
                if (a.hasValue(R.styleable.pnLoginInputRow_android_inputType)) {
                    mEditText.setInputType(a.getInteger(R.styleable.pnLoginInputRow_android_inputType, 0));
                }

                if (a.hasValue(R.styleable.pnLoginInputRow_android_imeOptions)) {
                    mEditText.setImeOptions(a.getInteger(R.styleable.pnLoginInputRow_android_imeOptions, 0));
                }

                if (a.hasValue(R.styleable.pnLoginInputRow_android_hint)) {
                    String hint = a.getString(R.styleable.pnLoginInputRow_android_hint);
                ...
                if (a.hasValue(R.styleable.pnLoginInputRow_passwordToggleEnabled)) {
                    mTextInputLayout.setPasswordVisibilityToggleEnabled(
                            a.getBoolean(R.styleable.pnLoginInputRow_passwordToggleEnabled, false));
                }

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    if (a.hasValue(R.styleable.pnLoginInputRow_android_textAlignment)) {
                        mEditText.setTextAlignment(
                                a.getInt(R.styleable.pnLoginInputRow_android_textAlignment, TEXT_ALIGNMENT_GRAVITY));
                    }
                }
            } finally {
                a.recycle();
