...
public class NemurSearchSuggestionAdapter extends CursorAdapter {
    ...
    public synchronized void setFilter(String filter) {
        ...
        Cursor sqlCursor = NemurSearchTable.getQueryStringCursor(filter, MAX_SUGGESTIONS);

        // create a MatrixCursor which will be the actual cursor behind this adapter
        MatrixCursor matrixCursor = new MatrixCursor(
                new String[]{
                        NemurSearchTable.COL_ID,
                        NemurSearchTable.COL_QUERY});

        if (sqlCursor.moveToFirst()) {
            // first populate the matrix from the db cursor...
            do {
                long id = sqlCursor.getLong(sqlCursor.getColumnIndex(NemurSearchTable.COL_ID));
                String query = sqlCursor.getString(sqlCursor.getColumnIndex(NemurSearchTable.COL_QUERY));
    ...
    public String getSuggestion(int position) {
        ...
        if (cursor != null) {
            return cursor.getString(cursor.getColumnIndex(NemurSearchTable.COL_QUERY));
        } else {
            return null;
        }
    }
    ...
    public View newView(Context context, Cursor cursor, ViewGroup parent) {
        View view = LayoutInflater.from(context).inflate(R.layout.nemur_listitem_suggestion, parent, false);
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        final String query = cursor.getString(cursor.getColumnIndex(NemurSearchTable.COL_QUERY));
